...
def run(path, epochs, lr):
    ...
    for epoch in range(epochs):
        ...
        print('loss of %3d -th opoch  %.3f %.2f:' % (epoch + 1, running_loss / 100, acc * 100), end=' # ')
        # testing phase
        correct = 0
        total = 0
        with torch.no_grad():
...
if __name__ == '__main__':
    path = '../../Data/Mnist-bags/mnist_1.mat'
    dataset_name = path.split('/')[-1]
    epochs = 10
    lr = 0.0001

    acc, f1 = run(path=path, epochs=epochs, lr=lr)
    print('-' * 20)
    print(dataset_name)
    print('epochs: ', epochs, end=', ')
    print('lr:', lr)
    print('acc: %.2f | f1: %.2f' % (acc * 100, f1 * 100))
